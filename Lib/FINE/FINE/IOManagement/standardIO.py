...
def writeOptimizationOutputToExcel(esM,
    ...
    :param output: type of file for the output. Either ['json', 'excel', or 'csv']
    """
    if output == 'excel':
        ...
        for name in esM.componentModelingDict.keys():
            ...
            if dataTD1dim:
                ...
                dfTD1dim = pd.concat(dataTD1dim, keys=indexTD1dim, names=names).T
                if oL_ == 1:
            ...
            if dataTD2dim:
                ...
                dfTD2dim = pd.concat(dataTD2dim, keys=indexTD2dim, names=names).T
                if oL_ == 1:
    ...
    if output == 'json':
        utils.output('\nWriting output to JSON... ', esM.verbose, 0)
        _t = time.time()
        ...
        for name in esM.componentModelingDict.keys():
            ...
            optSum = esM.getOptimizationSummary(name, outputLevel=oL_)
            if not optSum.empty:
                optSum.to_json(path_or_buf=(
                            outputFileName + "/" + name[:-5] + 'OptSummary_' + esM.componentModelingDict[
                        name].dimension + '.json'), orient='index', indent=2)
            ...
            if dataTD1dim:
                ...
                if oL_ == 1:
                    dfTD1dim = dfTD1dim.loc[((dfTD1dim != 0) & (~dfTD1dim.isnull())).any(axis=1)]
                if not dfTD1dim.empty:
                    dfTD1dim.to_json(path_or_buf=(outputFileName + "/" + name[:-5] + '_TDoptVar_1dim' + '.json'),
            ...
            if dataTD2dim:
                ...
                if oL_ == 1:
                    dfTD2dim = dfTD2dim.loc[((dfTD2dim != 0) & (~dfTD2dim.isnull())).any(axis=1)]
                if not dfTD2dim.empty:
                    dfTD2dim.to_json(path_or_buf=(outputFileName + "/" + name[:-5] + '_TDoptVar_2dim' + '.json'),
                                     orient='records', indent=2)
            if dataTI:
                if esM.componentModelingDict[name].dimension == '1dim':
                    names = ['Variable type', 'Component']
                ...
                if oL_ == 1:
                    dfTI = dfTI.loc[((dfTI != 0) & (~dfTI.isnull())).any(axis=1)]
                if not dfTI.empty:
                    dfTI.to_json(path_or_buf=(
                                outputFileName + "/" + name[:-5] + '_TIoptVar_' + esM.componentModelingDict[
                            name].dimension + '.json'), orient='records', indent=2)

        periodsOrder = pd.DataFrame([esM.periodsOrder], index=['periodsOrder'], columns=esM.periods)
        periodsOrder.to_json(path_or_buf=(outputFileName + "/" + 'Misc' + '.json'), orient='records', indent=2)
        if esM.segmentation:
            ls = []
            for i in esM.periodsOrder.tolist():
                ls.append(esM.timeStepsPerSegment[i])
            segmentDuration = pd.concat(ls, axis=1).rename(columns={"Segment Duration": "timeStepsPerSegment"})
            segmentDuration.index.name = 'segmentNumber'
            segmentDuration.to_json(path_or_buf=(outputFileName + "/" + 'Misc' + '.json'), orient='records', indent=2)
        utils.output('\tSaving file...', esM.verbose, 0)
        utils.output('Done. (%.4f' % (time.time() - _t) + ' sec)', esM.verbose, 0)
